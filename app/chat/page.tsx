...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    const newMessage = { user_id: userId, text: input };
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      else setMessages(data.map((m: any) => ({ user_id: m.user_id, text: m.text })));
    };
      ...
      <div className="flex-1 overflow-y-auto border rounded p-4 bg-white mb-4 h-[60vh]">
        {messages.map((m, idx) => (
          <div key={idx} className="mb-2">
            <span className="font-bold text-black">{m.user_id}: </span>
